...
var parseUrl = 'https://m.avito.ru/volgogradskaya_oblast_volzhskiy/kvartiry/prodam/vtorichka?f=549_5695-5696-5697.59_13984b13987.496_5120b5120.497_5182b5184.498_5244-5245-5246&i=1';
var jobs_list = [];
...
function after_clicked( page, job ) {
            job.title_obj = page.evaluate(function() {
                return document.querySelector('.single-item-header').innerText;
            });
    ...
    page.open(link, function (status) {
        if (status !== 'success') {
            ...
            var list = page.evaluate(function () {
                var job;
                var jobs = [];
                var objs = document.querySelectorAll('article.js-catalog-item-enum');
                    for (var i = 0; i < objs.length; i++) {
                        var title = objs[i].querySelector('h3');
                        var url = objs[i].querySelector('a');
                        job = {title: title.innerText, url: url.getAttribute('href'), phone: "", address: "", city: "", price: "", category: "", title_obj: "", contact_name: "", desc : "", person_name : ""};
                        jobs.push(job);
                    }
                return jobs;
            });
